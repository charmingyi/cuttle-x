...
export async function readSubscriptionSource(
  ...
  if (source.type === "collection") {
    // Collections carry no text of their own: delivery resolves them through their member pools.
    return {
      kind: "unavailable",
      error: new Error("A collection source must be resolved through its members."),
    }
  }
  if (source.type === "nodes") {
    // Nodes are resolved through the node repository in delivery.
    return {
      kind: "unavailable",
      error: new Error("A nodes source must be resolved through the node repository."),
    }
  }
  try {
