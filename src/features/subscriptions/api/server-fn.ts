...
export const reorderSubscriptions = createServerFn({ method: "POST" })
  .middleware([adminFunctionMiddleware])
  .validator((input: { ids: string[] }) => input)
  .handler(({ data }) => operations.reorderSubscriptions(data))
...
export const getSubscriptionLink = createServerFn({ method: "POST" })
  .middleware([adminFunctionMiddleware])
  .validator((input: { id: string }) => input)
  .handler(({ data }) => operations.getSubscriptionLink({ id: data.id, origin: getRequest().url }))

export const registerSubscriptionLink = createServerFn({ method: "POST" })
  .middleware([adminFunctionMiddleware])
  .validator((input: { id: string; link: string }) => input)
  .handler(({ data }) => operations.registerSubscriptionLink({ ...data, origin: getRequest().url }))
...
export const appendSubscriptionNodes = createServerFn({ method: "POST" })
  .middleware([adminFunctionMiddleware])
  .validator((input: { id: string; content: string }) => input)
  .handler(({ data }) => operations.appendSubscriptionNodes(data))
