...
export function SubscriptionDetailDialog({
  ...
  async function submitAppend() {
    ...
    try {
      await append.mutateAsync({ id: subscription.id, content: appendContent })
      setAppendContent("")
      setAppendOpen(false)
    } catch {
      // The mutation's onError already reports the failure; keep the input for retry.
    }
  }
  ...
  async function submitLegacyLink() {
    ...
    try {
      await registerLink.mutateAsync({ id: subscription.id, link: legacyLink })
      setLegacyLink("")
    } catch {
      // The mutation reports the failure and leaves the input available for correction.
    }
  }
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
          ...
          <div className="flex flex-col gap-2.5 p-4 md:px-6 md:py-4.5">
            {poolSubscription && appendOpen ? (
              <div className="flex flex-col gap-2.5 border-b bg-sidebar p-4 md:px-6 md:py-4.5">
                <span className={LABEL}>追加节点</span>
                <Textarea
                  aria-label="追加节点"
                  value={appendContent}
                ...
                <div className="flex justify-end gap-2">
                  <Button variant="outline" size="sm" onClick={() => setAppendOpen(false)}>
                    取消
                  </Button>
                  <Button
                    size="sm"
                  ...
                  >
                    {append.isPending ? "保存中" : "追加到订阅"}
                  </Button>
                </div>
              </div>
            ) : null}

            <div className="flex flex-col gap-2 border-b bg-sidebar p-3.5 md:p-4">
              <span className={LABEL}>登记或修复订阅链接</span>
