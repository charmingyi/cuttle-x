...
function request(credential?: string, ip = `198.51.100.${++requestId}`) {
...
describe("the door to subscription management", () => {
  ...
  test("whitespace around the presented credential is ignored too", async () => {
    ...
    expect(await authorizeAdminRequest(request("  abc", ip))).toBe("authorized")
    expect(await authorizeAdminRequest(request("abc  ", ip))).toBe("authorized")
  })

  test("temporarily rate-limits repeated wrong credentials per client address", async () => {
    withToken("abc")
    const ip = "198.51.100.241"
    for (let index = 0; index < 4; index += 1) {
      // oxlint-disable-next-line no-await-in-loop -- failures must be recorded in order for this test.
      expect(await authorizeAdminRequest(request(`wrong-${index}`, ip))).toBe("unauthorized")
    }
    expect(await authorizeAdminRequest(request("wrong-final", ip))).toBe("rate_limited")
    expect(await authorizeAdminRequest(request("abc", ip))).toBe("rate_limited")
  })
